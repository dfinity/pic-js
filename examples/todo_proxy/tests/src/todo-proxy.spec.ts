...
import { PocketIc, Actor, createIdentity, SubnetStateType } from '@dfinity/pic';
import { _SERVICE, idlFactory } from '../../../todo/declarations/todo.did';
...
const TODO_SUBNET_ID =
  'wwptv-65toc-emlst-l6axf-oy7nr-46aav-wakat-nskgm-bcmvl-bcgbd-sae';

const TODO_CANISTER_ID = Principal.fromText('bkyz2-fmaaa-aaaaa-qaaaq-cai');

const TODO_STATE_PATH = resolve(__dirname, '..', 'state', 'todo_state');
...
describe('Todo', () => {
  ...
  beforeEach(async () => {
    ...
    const stuff = await pic.getTopology();
    console.log('stuff', stuff);
    console.log('moar stuff', stuff[0].canisterRanges);
    console.log('moar stuff start', stuff[0].canisterRanges[0].start.toHex());
    console.log('moar stuff end', stuff[0].canisterRanges[0].end.toHex());
    console.log('moar stuff expected', TODO_CANISTER_ID.toHex());

    actor = pic.createActor<_SERVICE>(idlFactory, TODO_CANISTER_ID);

    await pic.setTime(new Date(2025, 4, 18));
    await pic.tick();
  });
  ...
  describe('with alice and bob', () => {
    ...
    it('should survive a reset and stable memory restore', async () => {
      ...
      const stableMemory = await pic.getStableMemory(TODO_CANISTER_ID);
      await pic.reinstallCode({
        canisterId: TODO_CANISTER_ID,
        wasm: WASM_PATH,
      });
      await pic.setStableMemory(TODO_CANISTER_ID, stableMemory);
    ...
    it('should stop and start the canister', async () => {
      ...
      await pic.stopCanister({ canisterId: TODO_CANISTER_ID });

      actor.setIdentity(alice);
      await expect(actor.get_todos()).rejects.toThrow(
        `Canister ${TODO_CANISTER_ID} is stopped`,
      );
      ...
      await pic.startCanister({ canisterId: TODO_CANISTER_ID });
