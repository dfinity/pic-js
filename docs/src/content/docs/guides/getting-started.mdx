...
`@icp-sdk/core` directly, it has to be on v6 as well. A version outside the range `@dfinity/pic`
requires resolves to a second, nested copy of core, and the two copies fail `instanceof` across the
API boundary:
