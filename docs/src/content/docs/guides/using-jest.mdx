...
The `transformIgnorePatterns` entry is required. PicJS depends on [`@icp-sdk/core`](https://www.npmjs.com/package/@icp-sdk/core), which in turn depends on `@noble/hashes` and `@noble/curves`. Those packages ship ES modules only, and Jest resolves the CommonJS build of `@icp-sdk/core`, so they have to be transformed instead of ignored along with the rest of `node_modules`. This also requires a `transform` that handles `.js` files, such as the one above. Without it, tests fail to load with one of these errors, depending on the Jest version:
...
```text
Must use import to load ES Module: .../node_modules/@noble/hashes/sha2.js
```

The pattern matches these packages at any depth, since npm and Yarn may nest them under a dependent. The second entry repeats Jest's default, which a custom `transformIgnorePatterns` replaces rather than extends. [Vitest](./using-vitest) loads ES modules natively and needs no equivalent setting.
