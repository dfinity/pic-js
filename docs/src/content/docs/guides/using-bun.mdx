...
To get started with [Bun](https://bun.sh) as a test runner, install the `@types/bun` and `typescript` packages using your preferred package manager:

<Tabs>
  <TabItem label="npm" icon="npm">
    <Code lang="shell" code="npm i -D @types/bun typescript" />
  </TabItem>
  <TabItem label="pnpm" icon="pnpm">
    <Code lang="shell" code="pnpm i -D @types/bun typescript" />
  </TabItem>
  <TabItem label="bun" icon="bun">
    <Code lang="shell" code="bun add -d @types/bun typescript" />
  </TabItem>
  <TabItem label="yarn" icon="seti:yarn">
    <Code lang="shell" code="yarn add -D @types/bun typescript" />
  </TabItem>
</Tabs>
...
You can also check out the official [Bun test runner documentation](https://bun.sh/docs/test) for more information on writing tests.

## As a package manager

PicJS downloads the `pocket-ic` binary from a `postinstall` script, which Bun blocks by default. Add `@dfinity/pic` as a [trusted dependency](https://bun.sh/docs/pm/lifecycle#trusteddependencies) in your `package.json` so the binary is fetched — see [allowing the install script](./getting-started#allowing-the-install-script) for the other package managers:
