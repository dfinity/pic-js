...
| `cyclesToken`   | The cycles ledger and index canisters                                                     |
| `nnsGovernance` | The NNS governance and root canisters, with an initial neuron staking 1 ICP               |
| `sns`           | The SNS-W and aggregator canisters, with the SNS canister WASMs uploaded                  |
| `ii`            | The Internet Identity canister                                                            |

The subnets that host these canisters must be empty, so don't load existing state into them. Enabling `cyclesMinting` also moves the instance's default time to 10 May 2021, to match the cycles minting canister's state.
